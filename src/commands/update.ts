...
async function performUpdate(fromVersion: string, toVersion: string, isNewVersion: boolean): Promise<void> {
  ...
  if (isGlobalInstall && !isNewVersion) {
    ...
    console.log('推荐的更新方式：')
    console.log()
    console.log(ansis.cyan('  npm install -g cogem-dualflow@latest'))
    console.log()
    ...
    if (useNpmUpdate) {
      ...
      console.log()
      console.log(ansis.cyan.bold('  npm install -g cogem-dualflow@latest'))
      console.log()
    ...
    console.log(ansis.gray('注意：这不会更新 cogem 命令本身'))
    console.log()
  }
  ...
  try {
    ...
    await execAsync(`npx --yes cogem-dualflow@latest --version`, { timeout: 60000 })
  ...
  try {
    ...
    await execAsync(`npx --yes cogem-dualflow@latest init --force --skip-mcp --skip-prompt`, {
      timeout: 120000,
      env: {
        ...process.env,
        COGEM_UPDATE_MODE: 'true', // Signal to init that this is an update
      },
    })
    ...
    console.log(ansis.cyan('  npx cogem-dualflow@latest'))
    return
  }
