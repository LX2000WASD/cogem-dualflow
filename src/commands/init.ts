...
import { createDefaultConfig, ensureCoGemConfigDir, readWorkflowConfig, writeWorkflowConfig } from '../utils/config'
...
export async function init(options: InitOptions = {}): Promise<void> {
  ...
  if (options.skipMcp) {
    ...
    if (selectedMcp === 'ace-tool' || selectedMcp === 'ace-tool-rs') {
      ...
      const { skipToken } = await inquirer.prompt([{
        type: 'confirm',
        name: 'skipToken',
        message: '是否跳过 Token 配置？（可稍后运行 npx cogem-dualflow config mcp 配置）',
        default: false,
      }])
      ...
      if (!skipToken) {
        ...
        console.log(ansis.gray(`     • 可稍后运行 ${ansis.cyan('npx cogem-dualflow config mcp')} 配置 Token`))
    ...
    // Configure ContextWeaver if selected
      ...
      const { skipKey } = await inquirer.prompt([{
        type: 'confirm',
        name: 'skipKey',
        message: '是否跳过 API Key 配置？（可稍后运行 npx cogem-dualflow config mcp 配置）',
        default: false,
      }])
      ...
      if (!skipKey) {
        ...
        console.log(ansis.gray(`     • 可稍后运行 ${ansis.cyan('npx cogem-dualflow config mcp')} 配置`))
  ...
  if (!options.skipPrompt) {
    ...
    const existingConfig = await readWorkflowConfig()
  ...
  try {
    ...
    await writeWorkflowConfig(config)

    // Install workflows and commands
    const installDir = options.installDir || join(homedir(), '.cogem')
    ...
    // Install ContextWeaver MCP if API key was provided
      ...
      console.log(`    ${ansis.gray('→')} 稍后运行 ${ansis.cyan('npx cogem-dualflow config mcp')} 完成配置`)
    }
    ...
    result.installedCommands.forEach((cmd) => {
      console.log(`    ${ansis.green('✓')} /cogem:${cmd}`)
    })
    ...
    if (result.binInstalled && result.binPath) {
      ...
      if (platform === 'win32') {
        ...
        try {
          ...
          if (!currentPathNorm.includes(windowsPathNorm) && !currentPathNorm.includes('.cogem\\bin')) {
        ...
        if (isZsh || isBash || isMacDefaultZsh) {
          ...
          try {
            ...
            if (rcContent.includes(result.binPath) || rcContent.includes('/.cogem/bin')) {
              console.log(`    ${ansis.green('✓')} PATH ${ansis.gray(`→ ${shellRcDisplay} (已配置)`)}`)
            }
            else {
              const configLine = `\n# CoGem multi-model collaboration system\n${exportCommand}\n`
