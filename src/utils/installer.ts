...
import { backupCoGemConfig, buildMcpServerConfig, fixWindowsMcpConfig, getCoGemConfigPath, mergeMcpServers, readCoGemConfig, writeCoGemConfig } from './mcp'
...
const WORKFLOW_CONFIGS: WorkflowConfig[] = [
  ...
  },
  {
    ...
    defaultSelected: true,
    order: 1.6,
    description: '根据计划获取原型 → Codex 重构实施 → 多模型审计交付',
    descriptionEn: 'Get prototype from plan → Codex refactor → Multi-model audit',
  },
  {
    id: 'frontend',
    name: '前端专项',
  ...
  },
  {
    ...
    defaultSelected: true,
    order: 10,
    description: '初始化项目 AI 上下文，生成 AGENTS.md',
    descriptionEn: 'Initialize project AI context, generate AGENTS.md',
  },
  {
    id: 'commit',
    name: 'Git 提交',
...
function replaceHomePathsInTemplate(content: string, installDir: string): string {
  ...
  const installRootDir = installDir
  ...
  // 1. Replace legacy and current prompt config roots
  processed = processed.replace(/~\/\.claude\/\.ccg/g, normalizePath(configDir))
  processed = processed.replace(/~\/\.cogem\/config/g, normalizePath(configDir))

  // 2. Replace legacy ~/.claude/bin/codeagent-wrapper with absolute path + .exe on Windows
  ...
  processed = processed.replace(/~\/\.cogem\/bin\/codeagent-wrapper/g, wrapperPath)

  // 3. Replace legacy ~/.claude/bin with absolute path (for other binaries)
  processed = processed.replace(/~\/\.claude\/bin/g, normalizePath(binDir))
  processed = processed.replace(/~\/\.cogem\/bin/g, normalizePath(binDir))

  // 4. Replace legacy ~/.claude with absolute path
  processed = processed.replace(/~\/\.claude/g, normalizePath(installRootDir))
  processed = processed.replace(/~\/\.cogem/g, normalizePath(installRootDir))
...
export async function installWorkflows(
  ...
  const commandsDir = join(installDir, 'commands', 'cogem')
  const configDir = join(installDir, 'config')
...
> This command is part of CoGem multi-model collaboration system.
`
  ...
  const agentsDestDir = join(installDir, 'agents', 'cogem')
  ...
  // Install skills (multi-model-collaboration, etc. - should go to ~/.cogem/skills/)
...
// Removed: getCoGemConfigPath - now imported from './mcp'

/**
 * Install and configure ace-tool MCP for CoGem CLI
 * Writes to ~/.cogem/config.json (the correct config file for CoGem CLI)
 */
...
export async function uninstallWorkflows(installDir: string): Promise<UninstallResult> {
  ...
  const commandsDir = join(installDir, 'commands', 'cogem')
  const promptsDir = join(installDir, 'config', 'prompts')
  const agentsDir = join(installDir, 'agents', 'cogem')
  const skillsDir = join(installDir, 'skills', 'multi-model-collaboration')
  const binDir = join(installDir, 'bin')
  const configDir = join(installDir, 'config')

  // Remove CoGem commands directory
  ...
  // Remove CoGem prompts directory (in config/) - handled by removing config dir below
  ...
  // Remove config directory (Force remove)
  if (await fs.pathExists(configDir)) {
    try {
      ...
      result.errors.push(`Failed to remove config directory: ${error}`)
...
/**
 * Uninstall ace-tool MCP configuration from ~/.cogem/config.json
 */
export async function uninstallAceTool(): Promise<{ success: boolean, message: string }> {
  try {
    const existingConfig = await readCoGemConfig()

    if (!existingConfig) {
      return {
        success: true,
        message: 'No ~/.cogem/config.json found, nothing to remove',
      }
    }
    ...
    await backupCoGemConfig()

    // Remove ace-tool from mcpServers
    delete existingConfig.mcpServers['ace-tool']

    // Write back
    await writeCoGemConfig(existingConfig)

    return {
      success: true,
      message: 'ace-tool MCP removed from ~/.cogem/config.json',
    }
  }
  catch (error) {
    return {
...
export async function installAceTool(config: AceToolConfig): Promise<{ success: boolean, message: string, configPath?: string }> {
  ...
  try {
    ...
    let existingConfig = await readCoGemConfig()
    ...
    if (existingConfig.mcpServers && Object.keys(existingConfig.mcpServers).length > 0) {
      const backupPath = await backupCoGemConfig()
      if (backupPath) {
    ...
    await writeCoGemConfig(mergedConfig)
...
/**
 * Install and configure ace-tool-rs MCP for CoGem CLI
...
/**
 * Install and configure ContextWeaver MCP for CoGem CLI
...
export async function installContextWeaver(config: ContextWeaverConfig): Promise<{ success: boolean, message: string, configPath?: string }> {
  ...
  try {
    ...
    const envContent = `# ContextWeaver 配置 (由 CoGem 自动生成)
...
`
    ...
    // 2. Read existing CoGem MCP config
    let existingConfig = await readCoGemConfig()
    if (!existingConfig) {
      existingConfig = { mcpServers: {} }
    }
...
/**
 * Uninstall ContextWeaver MCP from CoGem CLI
 */
...
/**
 * Install a generic MCP server to CoGem CLI
 */
...
/**
 * Uninstall a generic MCP server from CoGem CLI
 */
