 ...
 * Changes:
 * 1. ~/.ccg/ → ~/.cogem/config/
 * 2. ~/.claude/.ccg/ → ~/.cogem/config/
 * 3. ~/.claude/prompts/ccg/ → ~/.cogem/config/prompts/
 */
export async function migrateToV1_4_0(): Promise<MigrationResult> {
  ...
  const oldClaudeCcgDir = join(homedir(), '.claude', '.ccg')
  const newCcgDir = join(homedir(), '.cogem', 'config')
  ...
  try {
    ...
    if (await fs.pathExists(oldCcgDir)) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          result.migratedFiles.push(`~/.ccg/${file} → ~/.cogem/config/${file}`)
        }
        catch (error) {
    ...
    // 2. Migrate ~/.claude/.ccg/ → ~/.cogem/config/
    if (await fs.pathExists(oldClaudeCcgDir)) {
      const files = await fs.readdir(oldClaudeCcgDir)
      for (const file of files) {
        const srcFile = join(oldClaudeCcgDir, file)
        const destFile = join(newCcgDir, file)
        try {
          if (await fs.pathExists(destFile)) {
            result.skipped.push(`~/.claude/.ccg/${file} (already exists in new location)`)
            continue
          }
          await fs.copy(srcFile, destFile)
          result.migratedFiles.push(`~/.claude/.ccg/${file} → ~/.cogem/config/${file}`)
        }
        catch (error) {
          result.errors.push(`Failed to migrate from ~/.claude/.ccg/${file}: ${error}`)
          result.success = false
        }
      }
    }
    else {
      result.skipped.push('~/.claude/.ccg/ (does not exist, nothing to migrate)')
    }

    // 3. Migrate ~/.claude/prompts/ccg/ → ~/.cogem/config/prompts/
    if (await fs.pathExists(oldPromptsDir)) {
      try {
        ...
        if (await fs.pathExists(newPromptsDir)) {
          ...
          result.migratedFiles.push('~/.claude/prompts/ccg/ → ~/.cogem/config/prompts/')
...
export async function needsMigration(): Promise<boolean> {
  ...
  const hasOldClaudeCcgDir = await fs.pathExists(oldClaudeCcgDir)
  const hasOldPromptsDir = await fs.pathExists(oldPromptsDir)
  const hasOldConfigFile = await fs.pathExists(oldConfigFile)

  return hasOldCcgDir || hasOldClaudeCcgDir || hasOldPromptsDir || hasOldConfigFile
}
