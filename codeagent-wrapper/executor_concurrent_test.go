...
func TestExecutorHelperCoverage(t *testing.T) {
	...
	t.Run("generateFinalOutputAndArgs", func(t *testing.T) {
		...
		if !slices.Equal(args, []string{"e", "--skip-git-repo-check", "-C", "/tmp", "--json", "task"}) {
			t.Fatalf("unexpected codex args when approval required: %+v", args)
		}
	})
...
func TestExecutorExecuteConcurrentWithContextBranches(t *testing.T) {
	...
	t.Run("TestConcurrentTaskLoggerFailure", func(t *testing.T) {
		// Create a writable temp dir for the main logger, then flip TMPDIR to an invalid
		// non-directory path so task-specific loggers deterministically fail to open.
		writable := t.TempDir()
		...
		invalidTempRoot := filepath.Join(writable, "not-a-dir")
		if err := os.WriteFile(invalidTempRoot, []byte("x"), 0o600); err != nil {
			t.Fatalf("failed to create invalid temp root file: %v", err)
		}
