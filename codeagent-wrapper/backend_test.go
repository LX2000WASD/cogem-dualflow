...
func TestClaudeBuildArgs_GeminiAndCodexModes(t *testing.T) {
	...
	t.Run("codex build args includes bypass flag by default", func(t *testing.T) {
	...
	t.Run("codex build args omits bypass flag when approval required", func(t *testing.T) {
		const key = "CODEX_REQUIRE_APPROVAL"
		t.Cleanup(func() { os.Unsetenv(key) })
		os.Setenv(key, "true")
