...
func TestRunBuildCodexArgs_RequireApprovalEnvTrue(t *testing.T) {
	defer resetTestHooks()
	...
	if foundBypass {
		t.Fatalf("did not expect bypass flag when approval is required, got %v", args)
	}
}
...
func TestBackendBuildArgs_CodexBackend(t *testing.T) {
	t.Setenv("CODEX_REQUIRE_APPROVAL", "false")
