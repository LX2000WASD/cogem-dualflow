...
- **代码主权**：外部模型对文件系统**零写入权限**，所有修改由 Codex 执行
- **止损机制**：当前阶段输出通过验证前，不进入下一阶段
- **仅规划**：本命令允许读取上下文与写入 `.cogem/plan/*` 计划文件，但**禁止修改产品代码**
...
```
Bash({
  command: "~/.cogem/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
需求：<增强后的需求>
上下文：<检索到的项目上下文>
...
| 阶段 | Codex | Gemini |
|------|-------|--------|
| 分析 | `~/.cogem/config/prompts/codex/analyzer.md` | `~/.cogem/config/prompts/gemini/analyzer.md` |
| 规划 | `~/.cogem/config/prompts/codex/architect.md` | `~/.cogem/config/prompts/gemini/architect.md` |

**会话复用**：每次调用返回 `SESSION_ID: xxx`（通常由 wrapper 输出），**必须保存**以供后续 `/cogem:execute` 使用。
...
**Prompt 增强**（按 `/cogem:enhance` 的逻辑执行）：分析 $ARGUMENTS 的意图、缺失信息、隐含假设，补全为结构化需求（明确目标、技术约束、范围边界、验收标准），**用增强结果替代原始 $ARGUMENTS** 用于后续所有阶段。
...
1. **Codex 后端分析**：
   - ROLE_FILE: `~/.cogem/config/prompts/codex/analyzer.md`
   - 关注：技术可行性、架构影响、性能考量、潜在风险
   - OUTPUT: 多角度解决方案 + 优劣势分析

2. **Gemini 前端分析**：
   - ROLE_FILE: `~/.cogem/config/prompts/gemini/analyzer.md`
...
为降低主编排器合成计划的遗漏风险，可并行让两个模型输出“计划草案”（仍然**不允许**修改文件）：

1. **Codex 计划草案**（后端权威）：
   - ROLE_FILE: `~/.cogem/config/prompts/codex/architect.md`
   - OUTPUT: Step-by-step plan + pseudo-code（重点：数据流/边界条件/错误处理/测试策略）

2. **Gemini 计划草案**（前端权威）：
   - ROLE_FILE: `~/.cogem/config/prompts/gemini/architect.md`
...
**`/cogem:plan` 的职责到此结束，必须执行以下动作**：

1. 向用户展示完整实施计划（含伪代码）
2. 将计划保存至 `.cogem/plan/<功能名>.md`（功能名从需求中提取，如 `user-auth`、`payment-module` 等）
3. 以**加粗文本**输出提示（必须使用实际保存的文件路径）：

   ---
   **📋 计划已生成并保存至 `.cogem/plan/实际功能名.md`**
   ...
   **请审查上述计划，您可以：**
   ...
   ```
   /cogem:execute .cogem/plan/实际功能名.md
   ```
   ---
...
**⚠️ 绝对禁止**：
- ❌ 问用户 "Y/N" 然后自动执行（执行是 `/cogem:execute` 的职责）
- ❌ 对产品代码进行任何写操作
- ❌ 自动调用 `/cogem:execute` 或任何实施动作
- ❌ 在用户未明确要求修改时继续触发模型调用
...
- **首次规划**：`.cogem/plan/<功能名>.md`
- **迭代版本**：`.cogem/plan/<功能名>-v2.md`、`.cogem/plan/<功能名>-v3.md`...
...
```bash
/cogem:execute .cogem/plan/<功能名>.md
```
...
4. 外部模型对文件系统**零写入权限**
5. **SESSION_ID 交接** – 计划末尾必须包含 `CODEX_SESSION` / `GEMINI_SESSION`（供 `/cogem:execute resume <SESSION_ID>` 使用）
