---
description: '多模型协作执行 - 根据计划获取原型 → Codex 重构实施 → 多模型审计交付'
---
...
- **前置条件**：仅在用户对 `/cogem:plan` 输出明确回复 "Y" 后执行（如缺失，必须先二次确认）
...
Bash({
  command: "~/.cogem/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"{{WORKDIR}}\" <<'EOF'
ROLE_FILE: <角色提示词路径>
<TASK>
需求：<任务描述>
上下文：<计划内容 + 目标文件>
...
| 阶段 | Codex | Gemini |
|------|-------|--------|
| 实施 | `~/.cogem/config/prompts/codex/architect.md` | `~/.cogem/config/prompts/gemini/frontend.md` |
| 审查 | `~/.cogem/config/prompts/codex/reviewer.md` | `~/.cogem/config/prompts/gemini/reviewer.md` |

**会话复用**：如果 `/cogem:plan` 提供了 SESSION_ID，使用 `resume <SESSION_ID>` 复用上下文。
...
1. 调用 Gemini（使用 `~/.cogem/config/prompts/gemini/frontend.md`）
...
1. 调用 Codex（使用 `~/.cogem/config/prompts/codex/architect.md`）
2. 输入：计划内容 + 检索到的上下文 + 目标文件
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
1. **Codex 审查**（`run_in_background: true`）：
   - ROLE_FILE: `~/.cogem/config/prompts/codex/reviewer.md`
   - 输入：变更的 Diff + 目标文件
   - 关注：安全性、性能、错误处理、逻辑正确性

2. **Gemini 审查**（`run_in_background: true`）：
   - ROLE_FILE: `~/.cogem/config/prompts/gemini/reviewer.md`
   - 输入：变更的 Diff + 目标文件
   - 关注：可访问性、设计一致性、用户体验
...
1. `/cogem:plan` 生成计划 + SESSION_ID
2. 用户确认 "Y" 后
3. `/cogem:execute` 读取计划，复用 SESSION_ID，执行实施
