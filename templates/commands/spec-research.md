...
0. **MANDATORY: Enhance Requirement FIRST**
   ...
   - **Prompt 增强**（按 `/cogem:enhance` 的逻辑执行）：分析 $ARGUMENTS 的意图、缺失信息、隐含假设，补全为结构化需求（明确目标、技术约束、范围边界、验收标准）。
...
8. **Context Checkpoint**
   ...
   - If approaching 80K tokens, suggest: "Run `/clear` and continue with `/cogem:spec-plan`"
