...
**Guardrails**
- **前置条件**：`.cogem/team-plan/` 下必须有计划文件。没有则终止，提示先运行 `/cogem:team-plan`。
- **Agent Teams 必须启用**：需要 `COGEM_EXPERIMENTAL_AGENT_TEAMS=1`。
- Lead 绝不直接修改产品代码。
...
1. **前置检查**
   ...
   - 若不可用，输出启用指引后终止：
     ...
     在 settings.json 中添加：
     { "env": { "COGEM_EXPERIMENTAL_AGENT_TEAMS": "1" } }
     ```
   - 读取 `.cogem/team-plan/` 下最新的计划文件。
   - 若无计划文件，提示：`请先运行 /cogem:team-plan <任务描述> 生成计划`，终止。
