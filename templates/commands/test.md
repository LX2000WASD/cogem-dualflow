...
| 模型 | 提示词 |
|------|--------|
| Codex | `~/.cogem/config/prompts/codex/tester.md` |
| Gemini | `~/.cogem/config/prompts/gemini/tester.md` |
...
- **后端代码** → `Bash({ command: "...--backend codex...", run_in_background: false })`
  - ROLE_FILE: `~/.cogem/config/prompts/codex/tester.md`
- **前端代码** → `Bash({ command: "...--backend gemini...", run_in_background: false })`
  - ROLE_FILE: `~/.cogem/config/prompts/gemini/tester.md`
- **全栈代码** → 并行调用两者：
...
1. 收集模型输出
2. Codex 重构：统一风格、确保命名一致、优化结构、移除冗余
