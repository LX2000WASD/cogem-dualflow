...
1. **Prompt 增强**（按 `/cogem:enhance` 的逻辑执行）：分析 $ARGUMENTS 的意图、缺失信息、隐含假设，补全为结构化需求（明确目标、技术约束、范围边界、验收标准），**用增强结果替代原始 $ARGUMENTS，后续调用 Codex/Gemini 时传入增强后的需求**
...
**Codex 编排器综合规划**：采纳 Codex 后端规划 + Gemini 前端规划，用户批准后存入 `.cogem/plan/任务名.md`
...
1. 阶段顺序不可跳过（除非用户明确指令）
2. 外部模型对文件系统**零写入权限**，所有修改由 Codex 执行
3. 评分 <7 或用户未批准时**强制停止**
