...
**Prompt 增强**（按 `/cogem:enhance` 的逻辑执行）：分析 $ARGUMENTS 的意图、缺失信息、隐含假设，补全为结构化需求（明确目标、技术约束、范围边界、验收标准），**用增强结果替代原始 $ARGUMENTS，后续调用 Codex/Gemini 时传入增强后的需求**
...
```
执行 agent: ~/.cogem/agents/cogem/ui-ux-designer.md
输入: 项目上下文 + 用户需求 + 技术栈
输出: UI/UX 设计方案
```
...
```
执行 agent: ~/.cogem/agents/cogem/planner.md
...
- 迭代版本：`.cogem/plan/功能名-1.md`、`.cogem/plan/功能名-2.md`...
