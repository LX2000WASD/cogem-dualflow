...
- 不修改源代码；仅生成/更新文档与 `.cogem/index.json`。
- **忽略规则获取策略**：
...
1.  **写入根级 `AGENTS.md`**
    ...
    - 根级结构（精简而全局）：
      ...
      - **✨ 新增：模块结构图（Mermaid）**
        ...
        - 每个节点应可点击，并链接到对应模块的 `AGENTS.md` 文件。
        - 示例语法：
          ...
          graph TD
              ...
              click C "./packages/auth/AGENTS.md" "查看 auth 模块文档"
              click D "./packages/ui-library/AGENTS.md" "查看 ui-library 模块文档"
              click F "./services/audit-log/AGENTS.md" "查看 audit-log 模块文档"
          ```
...
2.  **写入模块级 `AGENTS.md`**
    - 放在每个模块目录下，结构建议：
      - **✨ 新增：相对路径面包屑**
        - 在每个模块 `AGENTS.md` 的**最顶部**，插入一行相对路径面包屑，链接到各级父目录及根 `AGENTS.md`。
        - 示例（位于 `packages/auth/AGENTS.md`）：
          `[根目录](../../AGENTS.md) > [packages](../) > **auth**`
      - 模块职责
      - 入口与启动
      - 对外接口
      - 关键依赖与配置
